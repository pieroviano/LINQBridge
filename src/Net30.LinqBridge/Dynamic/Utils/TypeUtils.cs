...
internal static class TypeUtils
{
    ...
    internal const MethodAttributes PublicStatic = MethodAttributes.FamANDAssem | MethodAttributes.Family |
        ...
        bool implicitOnly)
    {
        ...
        for (var i = 0; i < methodInfoArray.Length; i++)
        {
            ...
            if ((!(methodInfo.Name != "op_Implicit") || (!implicitOnly && !(methodInfo.Name != "op_Explicit"))) &&
                AreEquivalent(methodInfo.ReturnType, typeTo) &&
                AreEquivalent(methodInfo.GetParameters()[0].ParameterType, typeFrom))
            {
                return methodInfo;
            }
        }
    ...
    internal static MethodInfo GetBooleanOperator(Type type, string name)
    {
        do
        {
            var methodValidated = type.GetMethod(name,
                BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null, new[] { type }, null);
    ...
    internal static bool HasBuiltInEqualityOperator(Type left, Type right)
    {
        ...
        if (!left.IsValueType && !right.IsValueType &&
            (AreReferenceAssignable(left, right) || AreReferenceAssignable(right, left)))
        {
            return true;
        }
    ...
    {
        if (AreEquivalent(source, destination) || IsImplicitNumericConversion(source, destination) ||
            IsImplicitReferenceConversion(source, destination) || IsImplicitBoxingConversion(source, destination))
        {
            return true;
        }
    ...
    {
        return (t.GenericParameterAttributes & GenericParameterAttributes.Contravariant) !=
    ...
    {
        return (t.GenericParameterAttributes & GenericParameterAttributes.VarianceMask) ==
               GenericParameterAttributes.None;
    }
}
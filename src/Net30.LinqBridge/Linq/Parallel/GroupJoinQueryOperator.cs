    ...
    BinaryQueryOperator<TLeftInput, TRightInput, TOutput>
{
    ...
    {
        return CancellableEnumerable.Wrap(LeftChild.AsSequentialQuery(token), token).GroupJoin(
        ...
        for (var index = 0; index < partitionCount; ++index)
        {
            partitionedStream2[index] =
                new HashJoinQueryOperatorEnumerator<TLeftInput, TLeftKey, TRightInput, TKey, TOutput>(
                    leftHashStream[index], partitionedStream1[index], null, m_resultSelector, m_keyComparer,
                    cancellationToken);
        }

...
internal sealed class AsynchronousChannel<T> : IDisposable
{
    ...
    internal AsynchronousChannel(
        ...
        : this(index, 512 /*0x0200*/, chunkSize, cancellationToken, consumerEvent)
    {
    }
    ...
    internal bool IsFull
    {
        get
        {
            var producerBufferIndex = m_producerBufferIndex;
            var consumerBufferIndex = m_consumerBufferIndex;
            if (producerBufferIndex == consumerBufferIndex - 1)
            {
                return true;
            }

            return consumerBufferIndex == 0 && producerBufferIndex == m_buffer.Length - 1;
        }
    }
    ...
    {
        var producerChunkIndex = m_producerChunkIndex;
    ...
    private void FlushCachedChunk()
    {
        ...
        var objArray = new T[m_producerChunkIndex];
        Array.Copy(m_producerChunk, objArray, m_producerChunkIndex);
        EnqueueChunk(objArray);
    ...
    {
        var consumerBufferIndex = m_consumerBufferIndex;
        var objArray = m_buffer[consumerBufferIndex];
